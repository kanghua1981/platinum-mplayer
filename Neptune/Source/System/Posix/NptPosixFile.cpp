                  ...
                  NPT_Cardinal          max   /* = 0 */)
{
    ...
    for (;;) {
		...
		//NPT_File::GetInfo();
		/*add by kanghua ,add filter*/
		//printf("%s %d\n",(char*)NPT_FilePath::FileExtension(entry_pointer->d_name),
		//					entry_pointer->d_type);
		if (entry_pointer->d_type == DT_DIR)
		{
			entries.Add(NPT_String(entry_pointer->d_name));
		}
		else
		{
			for(int i=0;i<(sizeof(filterfilename)/32);i++)
			{
				if (0 == strncasecmp(filterfilename[i],NPT_FilePath::FileExtension(entry_pointer->d_name),32))
				{
					entries.Add(NPT_String(entry_pointer->d_name));
					break;
				}
