...
NPT_SET_LOCAL_LOGGER("platinum.media.server.audio.delegate")

/*----------------------------------------------------------------------
|   PLT_FileMediaServerDelegate::PLT_FileMediaServerDelegate
+---------------------------------------------------------------------*/
PLT_AudioMediaServerDelegate::PLT_AudioMediaServerDelegate(const char* url_root, 
...
|   PLT_FileMediaServerDelegate::~PLT_FileMediaServerDelegate
+---------------------------------------------------------------------*/
PLT_AudioMediaServerDelegate::~PLT_AudioMediaServerDelegate()
{
}

/*----------------------------------------------------------------------
|   PLT_AudioMediaServerDelegate::ProcessFileRequest
+---------------------------------------------------------------------*/
NPT_Result 
PLT_AudioMediaServerDelegate::ProcessFileRequest(NPT_HttpRequest&              request, 
...
+---------------------------------------------------------------------*/
NPT_Result 
PLT_AudioMediaServerDelegate::ServeFile(const NPT_HttpRequest&        request, 
                                       const NPT_HttpRequestContext& context,
                                       NPT_HttpResponse&             response,
...
+---------------------------------------------------------------------*/
NPT_Result
PLT_AudioMediaServerDelegate::OnBrowseMetadata(PLT_ActionReference&          action, 
...
/*----------------------------------------------------------------------
|   PLT_AudioMediaServerDelegate::OnBrowseDirectChildren
+---------------------------------------------------------------------*/
NPT_Result
PLT_AudioMediaServerDelegate::OnBrowseDirectChildren(PLT_ActionReference&          action, 
                                                    const char*                   object_id, 
                                                    const char*                   filter,
                                                    NPT_UInt32                    starting_index,
                                                    NPT_UInt32                    requested_count,
...
+---------------------------------------------------------------------*/
NPT_Result
PLT_AudioMediaServerDelegate::OnSearchContainer(PLT_ActionReference&          action, 
                                               const char*                   object_id, 
...
+---------------------------------------------------------------------*/
NPT_Result
PLT_AudioMediaServerDelegate::GetFilePath(const char* object_id, 
                                         NPT_String& filepath) 
{
...
+---------------------------------------------------------------------*/
NPT_String
PLT_AudioMediaServerDelegate::BuildSafeResourceUri(const NPT_HttpUrl& base_uri, 
                                                  const char*        host, 
...
+---------------------------------------------------------------------*/
NPT_Result
PLT_AudioMediaServerDelegate::ExtractResourcePath(const NPT_HttpUrl& url, 
                                                 NPT_String&        file_path)
{
...
+---------------------------------------------------------------------*/
NPT_String
PLT_AudioMediaServerDelegate::BuildResourceUri(const NPT_HttpUrl& base_uri, 
                                              const char*        host, 
                                              const char*        file_path)
{
...
+---------------------------------------------------------------------*/
PLT_MediaObject*
PLT_AudioMediaServerDelegate::BuildFromFilePath(const NPT_String&             filepath, 
                                               const PLT_HttpRequestContext& context,
                                               ...
                                               bool                          allip /* = false */)
{
    ...
    if (info.m_Type == NPT_FileInfo::FILE_TYPE_REGULAR) {
		...
		/*
        	object->m_Creator = "kanghua";

		object->m_Date = "2014";
		object->m_Description.description = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
		object->m_People.artists.Add("kkk");
		object->m_People.actors.Add("bbbb");
		object->m_People.authors.Add("aaaa");
		*/
