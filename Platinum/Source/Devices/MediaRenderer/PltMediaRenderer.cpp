                            ...
                            const PLT_HttpRequestContext& context)
{
	...
	if (name.Compare("GetTransportInfo", true) == 0) {
          return OnGetTransportInfo(action);
    }
	//printf("OnAction:%s\n",(char*)name);
...
PLT_MediaRenderer::OnGetPositionInfo(PLT_ActionReference& action)
{
	...
	NPT_String RelTime;
	NPT_String Track;
	NPT_String TrackDuration;
	NPT_String TrackURI;
	NPT_String TrackMetaData;
	NPT_String RelCount;
	NPT_String AbsCount;
	//printf("OnGetPositionInfo\n");
	FindServiceByName("AVTransport",service);
	service->GetStateVariableValue("RelativeTimePosition", RelTime);
	service->GetStateVariableValue("CurrentTrack", Track);
	service->GetStateVariableValue("CurrentTrackDuration", TrackDuration);
	service->GetStateVariableValue("AVTransportURI", TrackURI);
	service->GetStateVariableValue("AVTransportURIMetaData", TrackMetaData);
	service->GetStateVariableValue("RelativeCounterPosition", RelCount);
	service->GetStateVariableValue("AbsoluteCounterPosition", AbsCount);
	//printf("reltime %s\n",(char*)reltime);
	//printf("Track %s\n",(char*)Track);
	//printf("TrackURI %s\n",(char*)TrackURI);
	//printf("TrackMetaData %s\n",(char*)TrackMetaData);
	//printf("TrackDuration %s\n",(char*)TrackDuration);
	if (NPT_FAILED(action->SetArgumentValue("Track", Track))){
        return NPT_FAILURE;
    }
    if (NPT_FAILED(action->SetArgumentValue("TrackDuration", (char*)TrackDuration))) {
        return NPT_FAILURE;
    }
    if (NPT_FAILED(action->SetArgumentValue("TrackMetaData", (char*)TrackMetaData))) {
        return NPT_FAILURE;
    }
    if (NPT_FAILED(action->SetArgumentValue("TrackURI", (char*)TrackURI))) {
        return NPT_FAILURE;
    }
    if (NPT_FAILED(action->SetArgumentValue("RelTime", (char*)RelTime))) {
        return NPT_FAILURE;
    }
    if (NPT_FAILED(action->SetArgumentValue("AbsTime", (char*)RelTime))) {
        return NPT_FAILURE;
    }
	if (NPT_FAILED(action->SetArgumentValue("RelCount", (char*)RelCount))) {
        return NPT_FAILURE;
    }
    if (NPT_FAILED(action->SetArgumentValue("AbsCount", (char*)AbsCount))) {
        return NPT_FAILURE;
    }
...
NPT_Result 
PLT_MediaRenderer::OnGetTransportInfo(PLT_ActionReference& action)
{
	PLT_Service *service;
	NPT_String State;
	NPT_String Status;
	...
	service->GetStateVariableValue("TransportStatus", Status);
	service->GetStateVariableValue("TransportPlaySpeed", Speed);

	//printf("State %s\n",(char*)State);
	//printf("Status %s\n",(char*)Status);
	//printf("Speed %s\n",(char*)Speed);
	if (NPT_FAILED(action->SetArgumentValue("CurrentTransportState", (char*)State))) {
        return NPT_FAILURE;
    }
    if (NPT_FAILED(action->SetArgumentValue("CurrentTransportStatus", (char*)Status))) {
        return NPT_FAILURE;
    }
    if (NPT_FAILED(action->SetArgumentValue("CurrentSpeed", (char*)Speed))) {
        return NPT_FAILURE;
    }
